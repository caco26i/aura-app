...
Integration tests exercise auth, Zod validation, journey ownership, wrong-method and CORS preflight behavior, SOS **`429 rate_limited`** (hourly cap counts every POST, including validation failures), and append-only audit writes (see `test/api.integration.test.js`). They use `AURA_API_SKIP_LISTEN=1` and a temp audit file via env.
