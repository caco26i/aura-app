...
Integration tests exercise auth, Zod validation, journey ownership, wrong-method and CORS preflight behavior, rate-limit-adjacent routes, and append-only audit writes (see `test/api.integration.test.js`). They use `AURA_API_SKIP_LISTEN=1` and a temp audit file via env.
