...
| Clause | Verified in |
|--------|-------------|
| §4.1 Routes & shell | [`design/AURA_SCREEN_SPECS.md`](../../design/AURA_SCREEN_SPECS.md) (incl. deep links / `*` policy, **Modo Cita / Transporte / Check-in IA** wireframe subsection — `/cita` five shipped surfaces: encuentro draft, nudge banner `cita-checkin-nudge`, live timer region, optional browser notify, tab-title pulse) + `web/src/App.tsx`, `AppShell.tsx`; wireframes en [`ModoCita.tsx`](../src/pages/ModoCita.tsx), [`ModoTransporte.tsx`](../src/pages/ModoTransporte.tsx), [`CheckinInteligente.tsx`](../src/pages/CheckinInteligente.tsx) |
...
| Clause | Verified in |
|--------|-------------|
| §5 Accessibility (SOS, journey paths) | `RouteDocumentTitle` + `RouteAnnouncer` (`App.tsx`) — route-aware `document.title` (**Sign in · Aura** on `/auth`; including **Privacy & visibility** on `/settings#settings-privacy-and-visibility`) + polite `#route-announcer-status` on Settings navigations; `ScrollToTop` hash scroll + Privacy `h2` programmatic focus (`Settings.tsx`); `SkipToContent` + `#main-content` (`AppShell.tsx`, `Welcome.tsx`, `Emergency.tsx`, `Auth.tsx`); `Auth.tsx` (segment `role="tablist"`/`tab`, form `htmlFor`/`id`, `role="alert"` setup copy); `Emergency.tsx` (dialogs, `role` / `aria-*`), `JourneyActive.tsx`, `AuraMap.tsx` (`aria-busy`), `MapPage.tsx` (layer `role="switch"` `aria-checked` / `aria-pressed` + dynamic `aria-label` (on/off) + `aria-describedby` on toggles; **Find safest route (demo)** primary control with demo `aria-label` + `role="status"` feedback; `section` `aria-labelledby` / `aria-describedby` for map block), `Home.tsx` (visually hidden page `h1`; hub safe/alert `role="status"` + `aria-live="polite"`), `Settings.tsx` (`htmlFor`/`id` on safety inputs; location `fieldset` `aria-describedby` privacy note), `Trusted.tsx` (add form + saved rows `htmlFor`/`id`; permission `fieldset` `aria-describedby`; remove `aria-label`), `JourneyNew.tsx` (heading id + form `aria-describedby` lede), modo shells (`ModoCita` — `data-testid="cita-checkin-nudge"` nudge `role="status"` `aria-live="polite"` + live timer region `aria-labelledby="cita-checkin-live-label"` `aria-live="polite"` + optional browser notify + tab-title pulse vs `RouteDocumentTitle` base; **`ModoTransporte`** — `#transport-live-status` `role="status"` `aria-live="polite"` `aria-atomic="true"` + secciones `aria-labelledby`; **`CheckinInteligente`** — `#checkin-ia-status` igual + disparadores `aria-pressed` + historial vacío `role="status"`; `header` `aria-labelledby` + `h1` id en los tres); Playwright axe gate [`a11y-transport-checkin.spec.ts`](../e2e/a11y-transport-checkin.spec.ts) — `#main-content` scoped, includes `/auth` + `/welcome` step 0 ([AURA-365](/AURA/issues/AURA-365)) |
...
| §5 Visual consistency | [`design/AURA_DESIGN_SYSTEM.md`](../../design/AURA_DESIGN_SYSTEM.md) (`theme.css` + `aura-m3.css`), `theme.css`, `index.html` `theme-color` (mobile browser chrome) |
...
| 2026-06-29 | [AURA-286](/AURA/issues/AURA-286): Post–[AURA-278](/AURA/issues/AURA-278) wireframe PDR trace closure — **PASS** (`web/` `npm run lint` + production `vite build`); static cross-check of [`ModoTransporte.tsx`](../src/pages/ModoTransporte.tsx) + [`CheckinInteligente.tsx`](../src/pages/CheckinInteligente.tsx) vs [`AURA_SCREEN_SPECS.md`](../../design/AURA_SCREEN_SPECS.md) wireframe subsection, [`UX_EMPTY_LOADING_SAFETY.md`](./UX_EMPTY_LOADING_SAFETY.md) §1, and [`RouteDocumentTitle.tsx`](../src/components/RouteDocumentTitle.tsx) tab titles (**Modo transporte · Aura**, **Check-in IA · Aura**); no drift found. Gap register unchanged. |
...
| 2026-07-01 | [AURA-311](/AURA/issues/AURA-311): Design-token guardrails (post–[AURA-305](/AURA/issues/AURA-305)) — eng pass on AppShell/Home/modo shells: added `--aura-surface-soft` / `--aura-surface-muted` / `--aura-surface-faint` / `--aura-on-primary` to `theme.css`; replaced hard-coded rgba/hex in `Home.tsx`, `ModoCita.tsx`, `ModoTransporte.tsx`, `CheckinInteligente.tsx` with `theme.css` + `aura-m3.css` tokens (`--bg`, `--warn-c`/`--warn-br`, `--safe`/`--safe-c`, `--aura-radius-md`). `AppShell.tsx` + bottom nav (`.nbar`) already token-clean. `web/` lint + build PASS. |
| 2026-07-01 | [AURA-392](/AURA/issues/AURA-392): CEO **written pause confirmed** for [UX Designer II](/AURA/agents/ux-designer-ii) IC lane (post–[AURA-388](/AURA/issues/AURA-388)); [UX Designer](/AURA/agents/ux-designer) Lead UX also on stand-down. Resume triggers: growth doc ↔ shell alignment, Home notifications product decision, post-launch map tile retry UX — CEO assigns scoped child; no self-start. |
| 2026-07-01 | [AURA-388](/AURA/issues/AURA-388): **Full wireframe program closed** — Lead UX outside-shell sign-off **PASS** [AURA-382](/AURA/issues/AURA-382) (`/welcome`, `/auth`, `/emergency`). Combined with [AURA-371](/AURA/issues/AURA-371) in-shell acceptance: all routes in [`AURA_SCREEN_SPECS.md`](../../design/AURA_SCREEN_SPECS.md) wireframe-documented + Lead UX accepted. Gap register unchanged; UX II IC lane **written pause**. |
| 2026-07-01 | [AURA-376](/AURA/issues/AURA-376): Outside-shell wireframe spec pass — **PASS** (doc-only): [`AURA_SCREEN_SPECS.md`](../../design/AURA_SCREEN_SPECS.md) wireframe subsection for `/welcome`, `/auth`, `/emergency` (onboarding carousel, auth card + tabs, SOS visible/silent confirm flows, tab titles **Welcome · Aura** / **Sign in · Aura** / **Emergency · Aura**). No CTO child. Gap register unchanged. |
| 2026-07-01 | [AURA-371](/AURA/issues/AURA-371): Wireframe program **closed** — Lead UX sign-off **PASS**: [AURA-358](/AURA/issues/AURA-358) Settings + [AURA-359](/AURA/issues/AURA-359) Journey accepted (no eng child). Full chain: UX II spec passes + Lead UX acceptance for Home ([AURA-310](/AURA/issues/AURA-310)), Map+Trusted ([AURA-333](/AURA/issues/AURA-333)), Settings, Journey. Gap register unchanged; UX II IC lane idle. |
| 2026-07-01 | [AURA-360](/AURA/issues/AURA-360): Lead UX sign-off prep — PDR trace rows for pending acceptance: [AURA-358](/AURA/issues/AURA-358) (Settings wireframe, [`AURA_SCREEN_SPECS.md`](../../design/AURA_SCREEN_SPECS.md) + [`UX_ONBOARDING_TRUST_SETTINGS.md`](./UX_ONBOARDING_TRUST_SETTINGS.md) §4) and [AURA-359](/AURA/issues/AURA-359) (Journey wireframe, [`UX_EMPTY_LOADING_SAFETY.md`](./UX_EMPTY_LOADING_SAFETY.md) §1). UX II IC lane hands off to [UX Designer](/AURA/agents/ux-designer). |
| 2026-07-01 | [AURA-346](/AURA/issues/AURA-346): IC lane **written pause** — in-shell wireframe audit program complete (Home, Journey, Map, Trusted, Settings, Modo Cita/Transport/Check-in in [`AURA_SCREEN_SPECS.md`](../../design/AURA_SCREEN_SPECS.md)); gap register unchanged; [UX Designer II](/AURA/agents/ux-designer-ii) idle until CEO assigns new scope. Outside-shell `/welcome`, `/auth`, `/emergency` covered by existing UX docs. |
| 2026-07-01 | [AURA-336](/AURA/issues/AURA-336): Settings wireframe spec pass — **PASS** (doc-only): [`AURA_SCREEN_SPECS.md`](../../design/AURA_SCREEN_SPECS.md) wireframe subsection for `/settings` (device data block, safety defaults, privacy deep link, BFF session, clear-local dialog); [`UX_EMPTY_LOADING_SAFETY.md`](./UX_EMPTY_LOADING_SAFETY.md) §1 inventory row expanded. No CTO child. Gap register unchanged. |
| 2026-07-01 | [AURA-316](/AURA/issues/AURA-316): Journey flow wireframe spec pass — **PASS** (doc-only): [`AURA_SCREEN_SPECS.md`](../../design/AURA_SCREEN_SPECS.md) wireframe subsection for `/journey/new` + `/journey/active` (form, share primer, silent sheet, end-journey confirm, tab titles **New journey · Aura** / **Live tracking · Aura**); [`UX_EMPTY_LOADING_SAFETY.md`](./UX_EMPTY_LOADING_SAFETY.md) §1 inventory rows expanded. No CTO child. Gap register unchanged. |
| 2026-07-01 | [AURA-309](/AURA/issues/AURA-309): Home hub wireframe spec pass — **PASS** (doc-only): [`AURA_SCREEN_SPECS.md`](../../design/AURA_SCREEN_SPECS.md) wireframe subsection for `/` (`Home.tsx`: header, global status live region, `monitorLine`, journey CTA, SOS tile + feature grids, footer links, tab title **Home · Aura**); [`UX_EMPTY_LOADING_SAFETY.md`](./UX_EMPTY_LOADING_SAFETY.md) §1 inventory row expanded. No CTO child. Gap register unchanged. |
| 2026-07-01 | [AURA-329](/AURA/issues/AURA-329): Map + Trusted wireframe spec pass — **PASS** (doc-only, `main` @ `0401060`): static cross-check of [`MapPage.tsx`](../src/pages/MapPage.tsx) + [`Trusted.tsx`](../src/pages/Trusted.tsx) vs [`AURA_SCREEN_SPECS.md`](../../design/AURA_SCREEN_SPECS.md) wireframe subsections + §7 Trusted row, [`UX_EMPTY_LOADING_SAFETY.md`](./UX_EMPTY_LOADING_SAFETY.md) §1, [`RouteDocumentTitle.tsx`](../src/components/RouteDocumentTitle.tsx) tab titles (**Map intel · Aura**, **Trusted contacts · Aura**); no eng drift. Gap register unchanged. |
...
| 2026-07-01 | [AURA-331](/AURA/issues/AURA-331): Auth + map layer design-token pass (post–[AURA-321](/AURA/issues/AURA-321)) — tokenized [`Auth.tsx`](../src/pages/Auth.tsx) (`--aura-surface-raised`, `--aura-shadow-lg`, `--aura-shadow-brand`, `--bg`, `--aura-on-primary`) and [`AuraMap.tsx`](../src/components/AuraMap.tsx) (tile overlay `--aura-card`; marker colors from `--aura-status-alert` / `--aura-status-ok` / `--aura-map-activity` via `readMapIntelColors()`); new tokens in [`theme.css`](../src/theme.css). [AURA-305](/AURA/issues/AURA-305) exceptions untouched. `web/` lint + build PASS. |
| 2026-07-01 | [AURA-353](/AURA/issues/AURA-353): MapPage layer-toggle design tokens (post–[AURA-331](/AURA/issues/AURA-331)) — replaced inline hex in [`MapPage.tsx`](../src/pages/MapPage.tsx) layer `role="switch"` controls: on-track `--aura-status-ok`, off-track `--s3` (`aura-m3.css`), thumb `--aura-on-primary`; a11y attrs unchanged. [AURA-305](/AURA/issues/AURA-305) exceptions untouched. `web/` lint + build PASS. |
| 2026-07-01 | [AURA-364](/AURA/issues/AURA-364): JourneyActive action-sheet design tokens (post–[AURA-353](/AURA/issues/AURA-353)) — tokenized [`JourneyActive.tsx`](../src/pages/JourneyActive.tsx) share primer, silent alert sheet, and end-journey confirm: card `--aura-surface-raised`, secondary/cancel `--aura-card`, destructive label `--aura-on-primary`; overlay scrim remains inline `rgba(0,0,0,0.45)` (documented in [`AURA_DESIGN_SYSTEM.md`](../../design/AURA_DESIGN_SYSTEM.md)). [AURA-305](/AURA/issues/AURA-305) other exceptions untouched. `web/` lint + build PASS. |
| 2026-07-01 | [AURA-384](/AURA/issues/AURA-384): JourneyActive overlay scrim design token (post–[AURA-372](/AURA/issues/AURA-372)) — added `--aura-overlay-scrim` to [`theme.css`](../src/theme.css); replaced 3 inline `rgba(0,0,0,0.45)` scrims in [`JourneyActive.tsx`](../src/pages/JourneyActive.tsx); updated [`AURA_DESIGN_SYSTEM.md`](../../design/AURA_DESIGN_SYSTEM.md). [AURA-305](/AURA/issues/AURA-305) exceptions untouched. `web/` lint + build PASS. |
| 2026-07-01 | [AURA-391](/AURA/issues/AURA-391): JourneyNew + AuraErrorBoundary residual design tokens (post–[AURA-384](/AURA/issues/AURA-384)) — tokenized [`JourneyNew.tsx`](../src/pages/JourneyNew.tsx) error alert `--aura-status-alert`, input `--aura-surface-raised`; [`AuraErrorBoundary.tsx`](../src/components/AuraErrorBoundary.tsx) retry label `--aura-text`. JourneyNew gradient CTA ([AURA-305](/AURA/issues/AURA-305) exception) untouched. `web/` lint + build PASS. |
| 2026-07-01 | [AURA-372](/AURA/issues/AURA-372): Welcome + SOS FAB on-primary tokens (post–[AURA-364](/AURA/issues/AURA-364)) — replaced inline `#fff` with `--aura-on-primary` on [`Welcome.tsx`](../src/pages/Welcome.tsx) primary CTA and [`AuraSOSButton.tsx`](../src/components/AuraSOSButton.tsx) FAB label. [AURA-305](/AURA/issues/AURA-305) exceptions untouched. `web/` lint + build PASS. |
| 2026-07-01 | [AURA-365](/AURA/issues/AURA-365): Axe e2e for `/auth` + `/welcome` (post–[AURA-349](/AURA/issues/AURA-349)) — added `#main-content`-scoped axe tests in [`a11y-transport-checkin.spec.ts`](../e2e/a11y-transport-checkin.spec.ts): `/auth` (Aura account h1, Email/Password labels, tablist); `/welcome` step 0 with `onboardingCompleted: false` bootstrap. **Zero violations** on both routes. §5 trace row updated. `web/` lint + `npm run test:e2e -- a11y-transport-checkin` **12 passed**. |
| 2026-07-01 | [AURA-377](/AURA/issues/AURA-377): Wireframe routes API contract static sweep — **PASS** (post–[AURA-375](/AURA/issues/AURA-375); CTO scope [comment 2c1626f6](/AURA/issues/AURA-377#comment-2c1626f6-f4bb-4613-8fed-e03df15ceab3)): `/transport`, `/checkin`, `/cita`, `/welcome` + modo shells [`ModoCita.tsx`](../src/pages/ModoCita.tsx), [`ModoTransporte.tsx`](../src/pages/ModoTransporte.tsx), [`CheckinInteligente.tsx`](../src/pages/CheckinInteligente.tsx) — no `auraBackend.ts` / `remotePost` / `/v1/*` (local `AuraContext` only). `/auth` — documented BFF only ([`Auth.tsx`](../src/pages/Auth.tsx) → [`auraBackendAuth.ts`](../src/api/auraBackendAuth.ts) `/auth/firebase`; no `/v1/*`). Journey / SOS / settings beta API unchanged ([`JourneyNew.tsx`](../src/pages/JourneyNew.tsx), [`JourneyActive.tsx`](../src/pages/JourneyActive.tsx), [`Emergency.tsx`](../src/pages/Emergency.tsx), [`Settings.tsx`](../src/pages/Settings.tsx)) per [API_CONTRACT.md](./API_CONTRACT.md). [BETA_BACKEND.md](./BETA_BACKEND.md) local-only route table added. Gap register unchanged. |
