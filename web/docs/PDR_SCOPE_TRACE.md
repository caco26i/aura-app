...
| Clause | Verified in |
|--------|-------------|
| §4.1 Routes & shell | [`design/AURA_SCREEN_SPECS.md`](../../design/AURA_SCREEN_SPECS.md) (incl. deep links / `*` policy) + `web/src/App.tsx`, `AppShell.tsx`, primary-nav modo pages (`ModoCita.tsx`, `ModoTransporte.tsx`, `CheckinInteligente.tsx`) |
...
| Date | Change |
|------|--------|
| 2026-04-08 | [AURA-269](/AURA/issues/AURA-269): **`/transport`** + **`/checkin`** wireframes (verify / desvío / «No soy yo»; disparadores, respuestas rápidas, historial local + polite live region); [`AURA_SCREEN_SPECS.md`](../../design/AURA_SCREEN_SPECS.md) §Primary nav + §4.1 trace row (`ModoTransporte.tsx`, `CheckinInteligente.tsx`). |
