...
| 2026-04-08 | [AURA-197](/AURA/issues/AURA-197): post-launch spec drift audit — [`AURA_SCREEN_SPECS.md`](../../design/AURA_SCREEN_SPECS.md) route table + deep-link gate now include **`/auth`**; tab-title note for `/auth`; [`PUBLIC_BETA.md`](../../docs/PUBLIC_BETA.md) §3 flows aligned with shipped shell (nav + map + auth); [`AURA_LAUNCH_UX.md`](../../design/AURA_LAUNCH_UX.md) offline copy table aligned with `auraApiMessages.ts` journey vs SOS strings. |
