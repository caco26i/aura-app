...
- **Web container (static + nginx):** build from [`web/Dockerfile`](../Dockerfile) — multi-stage `npm ci` / `npm run build`, then `nginx:alpine` with `try_files` SPA fallback. Pass **`VITE_*` as build-args** (they are not available at nginx runtime). For a **single Compose stack** (web + API, same Docker network), use the repo root [`docker-compose.yml`](../../docker-compose.yml): nginx proxies `/v1/`, `/health`, and `/ready` to the `aura-api` service so the browser can keep **`VITE_AURA_API_URL` unset** (same-origin `/v1`, matching local Vite proxy behavior). **Static-token stack:** set `AURA_API_BEARER_TOKEN` in `.env`; the compose file can pass the same value into the web image as `VITE_AURA_API_TOKEN` at build time — **rebuild the web image** when you rotate that token. **JWT + BFF stack:** omit `VITE_AURA_API_TOKEN`, deploy BFF, add an nginx `location` proxying your chosen path to the BFF, and set **`VITE_AURA_BFF_URL`** at web build time. Optional: `WEB_PORT`, `API_PORT`, `VITE_GOOGLE_CLIENT_ID`, `VITE_AURA_BFF_URL`, `VITE_AURA_TELEMETRY_*` in `.env` for the web build args.
