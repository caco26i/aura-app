...
5. In DevTools **Network**, confirm credentialed **`GET /session`** (or the app’s equivalent session fetch under your BFF prefix) returns **`200`** with an access token, then confirm authenticated **`/v1/*`** requests succeed — e.g. **`POST /v1/journeys`** when starting a journey and **`POST /v1/journeys/:id/im-safe`** when tapping **I'm safe** on `/journey/active` — returning **`200`** / expected success, not **`401`** from missing or invalid auth.
