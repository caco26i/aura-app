...
| Correlation | Optional `X-Request-Id` or `X-Correlation-Id` — printable ASCII, max **128** chars; server echoes **`X-Request-Id`** on every response (generated UUID when omitted or invalid). The in-repo BFF (`server/bff`) follows the same rules on its routes. For **credentialed cross-origin** SPA → BFF calls, CORS **`Access-Control-Expose-Headers`** includes **`X-Request-Id`** only (incoming **`X-Correlation-Id`** still selects the echoed id). |
| Content-Type | `application/json` on POST bodies when a body is sent |
| JSON body size | Default **32kb** per route (`express.json`). **Authoritative API:** override with **`AURA_API_JSON_BODY_LIMIT`**. **In-repo BFF:** override with **`AURA_BFF_JSON_BODY_LIMIT`** (same default; see [`../../server/bff/README.md`](../../server/bff/README.md)). |
...
| Read-path rate limits (unauthenticated GET) | Per-IP **`AURA_API_RATE_LIMIT_READ_WINDOW_MS`** / **`AURA_API_RATE_LIMIT_READ_MAX`** applies to **`GET /health`**, **`GET /ready`**, and (when metrics are enabled) **`GET /metrics`** in one shared bucket. **429** → `rate_limited`; audit `route` is **`health`**, **`ready`**, or **`metrics`**. Disable only with **`AURA_API_RATE_LIMIT_READ_SKIP=1`** (see README). |
...
| `GET /metrics` | **Operator-only, opt-in:** when `AURA_API_PROMETHEUS_METRICS` is enabled on the server, **200** with **`Content-Type: text/plain; version=0.0.4`** (Prometheus exposition). **No** `Authorization` header. When the env flag is **not** set (default), the path is unknown → **404** `not_found` with the usual JSON error envelope. |

**Note:** `/health` and `/ready` are documented JSON successes without `{ ok, data }`; the web app does not call them through `auraBackend.ts`. **`GET /metrics`** is not used by the SPA; it exists for internal scrapers when explicitly enabled.
...
| 429 | `rate_limited` | express-rate-limit — **minute** windows (`globalLimiter` / `journeyLimiter` on mutating routes, read-path limiter on **`GET /health`** / **`GET /ready`** / **`GET /metrics`**) or **hourly** caps (SOS, location-shares, im-safe); JSON envelope `{ ok: false, error, detail }`; `audit.rate_limited` with a `route` label |
...
**In-repo BFF (`server/bff`):** its **`GET /ready`** uses the same **`not_ready`** JSON shape when **`AURA_API_BFF_JWT_SECRET`** or **`AURA_BFF_SESSION_SECRET`** is missing or shorter than 16 characters — it does **not** mirror the API’s static-bearer or audit-directory checks. Misconfigured Google/Firebase auth routes can return **`bff_misconfigured`**, **`firebase_not_configured`**, or **`invalid_token`**; see [`../../server/bff/README.md`](../../server/bff/README.md).
...
Server integration tests: `server/test/api.integration.test.js`, `server/test/api.rate-limit-minute.integration.test.js`, `server/test/api.rate-limit-global.integration.test.js`, `server/test/api.rate-limit-read.integration.test.js`, `server/test/journey-registry.restart.test.js`, `server/test/journey-http-restart.integration.test.js`, and `server/test/prometheus-metrics.integration.test.js` (`npm test` in `server/`). Includes **BFF JWT** path (`AURA_API_BFF_JWT_SECRET` in tests): same **`sub`** across two tokens can **`im-safe`** on an owned journey; invalid signature / expired **`exp`** → **`403`**. Also: `invalid_journey_id` on `:journeyId` routes, **`im-safe` non-empty body → `validation_failed`**, malformed JSON → **`invalid_json`**, **oversized JSON body → `413` `payload_too_large`** on journey and SOS routes (default limit), **`401`** for non-`Bearer` `Authorization`, unknown-path / wrong-method **`not_found`**, **OPTIONS** CORS preflight, **`403` `journey_forbidden`** when a second static bearer (`AURA_API_BEARER_TOKEN_ALT`) calls **location-share** or **`im-safe`** on another actor’s journey, baseline **security headers** on `GET /health` and sample **error** responses (`404`, `401`), **`429` `rate_limited`** on **`POST /v1/emergency-alerts`** after the hourly SOS cap (emergency tests grouped at the **end** of the main suite; `audit.rate_limited` in the audit file), **`429`** when the **minute-window global** limiter trips on **`POST /v1/emergency-alerts`** (dedicated env-tuned file; proves abuse cannot skip global caps by avoiding journey routes), **`429`** when the **minute-window journey** limiter trips on **`POST /v1/journeys`** (dedicated env-tuned test file), **`429`** when the **read-path** limiter trips on **`GET /health`**. **im-safe** has an additional hourly cap. Production: prefer **`AURA_API_BFF_JWT_SECRET`** without static bearer in the browser; optional `AURA_API_BEARER_TOKEN_ALT` for two static actors in staging.
