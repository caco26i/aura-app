...
- **`/cita` (`ModoCita.tsx`)** — Local-only wireframe (primary bottom nav): borrador de encuentro en `aura:v1` (nombre, lugar, palabra de seguridad, hora `datetime-local`, intervalo de check-in); foto de contexto **deshabilitada** (marcador de diseño). **Check-in sugerido:** banner `role="status"` + `aria-live="polite"` cuando toca el intervalo local; CTA *Listo, seguir* persiste `encuentroLastLocalCheckInAckMs`. Región viva del temporizador con `role="region"`, `aria-labelledby="cita-checkin-live-label"`, `aria-live="polite"` (copy dinámico de cuenta regresiva / próximo recordatorio). Notificaciones del navegador **opcionales** (solo pestaña en segundo plano, permiso concedido). Tab title base **`Modo Cita · Aura`** via `RouteDocumentTitle`; pulso **`• Check-in ·`** en `document.title` mientras el nudge está activo y la pestaña visible. `header` `aria-labelledby="modo-cita-title"`. Sin SMS/API hasta contrato futuro. Tono calmado alineado a [`AURA_LAUNCH_UX.md`](./AURA_LAUNCH_UX.md).
