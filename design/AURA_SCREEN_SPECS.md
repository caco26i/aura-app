...
- **Tab titles** — `RouteDocumentTitle` maps each shell route, `/welcome`, `/emergency`, and **`/auth`** (`Sign in · Aura`) to a dedicated `document.title` (including **`/transport`** → **Modo transporte · Aura** and **`/checkin`** → **Check-in IA · Aura**). Announcer copy for `/auth` is unchanged (only `/settings` hash paths are announced today).
